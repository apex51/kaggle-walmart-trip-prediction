...
m_train_department_return 51 cols
m_train_fineline_buy 5323 cols
m_train_fineline_return 3936 cols
'''
feat_train = np.hstack((week_train, numbuy_train, numret_train, kindbuy_train, kindret_train, onlybuy_train, onlyret_train, retnbuy_train))
feat_test = np.hstack((week_test, numbuy_test, numret_test, kindbuy_test, kindret_test, onlybuy_test, onlyret_test, retnbuy_test))
feat_train = sparse.csr_matrix(feat_train)
feat_test = sparse.csr_matrix(feat_test)

# only thi part need change
x_train = hstack((feat_train, m_train_department_buy, m_train_department_return), 'csr')
x_test = hstack((feat_test, m_test_department_buy, m_test_department_return), 'csr')
...
# over sample for class 8 because class 8 only have 4 samples, add one to 5, for 5 fold use
print 'over sampling for training data class 8'
x_train = vstack((x_train, x_train[y_train == 8][0]), 'csr')
y_train = np.append(y_train, 8)

# dump to file
with open('../data/features/pack_count_no_fineline.pkl', 'wb') as f:
