...
clf = AdaBoostClassifier(RandomForestClassifier(n_estimators=10, n_jobs=-1), n_estimators=10000)
clf.fit(x_train_sp, y_train_sp)
pred = clf.predict_proba(x_test_sp)
print log_loss(y_test_sp, pred)

# # calib random forest for test
# from sklearn.ensemble import RandomForestClassifier
# from sklearn.calibration import CalibratedClassifierCV

# clf = CalibratedClassifierCV(RandomForestClassifier(n_estimators=100, n_jobs=-1), cv=2, method='isotonic')
# clf.fit(x_train_sp, y_train_sp)
# pred = clf.predict_proba(x_test_sp)
# print log_loss(y_test_sp, pred)
